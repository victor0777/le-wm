"""Track 2 V2: Dual-token conditioned training (maneuver + topology).

Combines maneuver pseudo-labels (6 classes) with topology tokens (60 classes)
to provide richer route-invariant conditioning to the predictor.

Both token embeddings are ADDED to the action embedding:
    predictor input = action_emb + maneuver_emb + topology_emb

Usage:
    # V3-topo: Livlab 4 train -> da8241 holdout (cross-domain reverse)
    CUDA_VISIBLE_DEVICES=2 PYTHONPATH=/home/ktl/projects/le-wm python train_vp_token_v2.py \
        data=rtb_occany \
        +holdout=Gian-Pankyo_JT_2025-08-19_04-59-28_2111_da8241 \
        wandb.enabled=False trainer.devices=1 \
        subdir=track2_v3_topo output_model_name=lewm_track2_v3_topo
"""
...
MANEUVER_LABELS_DIR = Path.home() / ".stable_worldmodel" / "rtb_occany_labels"
TOPOLOGY_LABELS_DIR = Path.home() / ".stable_worldmodel" / "rtb_occany_topology"
NUM_MANEUVER_CLASSES = 6   # left=0, right=1, straight=2, stop=3, accel=4, decel=5
NUM_TOPOLOGY_CLASSES = 60  # 3 curvature * 4 speed * 5 dynamics


class DualTokenLabelWrapper(torch.utils.data.Dataset):
    """Wraps HDF5Dataset to inject both maneuver and topology labels."""

    def __init__(self, hdf5_dataset: swm.data.HDF5Dataset,
                 maneuver_dir: Path = MANEUVER_LABELS_DIR,
                 topology_dir: Path = TOPOLOGY_LABELS_DIR):
        self.ds = hdf5_dataset
        self.num_steps = hdf5_dataset.num_steps
        self.frameskip = hdf5_dataset.frameskip

        h5_stem = hdf5_dataset.h5_path.stem
        short_id = h5_stem.split("_")[-1]

        # Load maneuver labels
        maneuver_path = maneuver_dir / f"{short_id}_labels.npz"
        if not maneuver_path.exists():
            raise FileNotFoundError(f"Maneuver labels not found: {maneuver_path}")
        self.maneuver_labels = np.load(maneuver_path, allow_pickle=True)["labels"].astype(np.int64)

        # Load topology labels
        topology_path = topology_dir / f"{short_id}_topology.npz"
        if not topology_path.exists():
            raise FileNotFoundError(f"Topology labels not found: {topology_path}")
        topo_data = np.load(topology_path, allow_pickle=True)
        self.topology_labels = topo_data["combined"].astype(np.int64)

        # Validate
        with h5py.File(hdf5_dataset.h5_path, "r") as f:
            total_frames = f["pixels"].shape[0]

        for name, arr in [("maneuver", self.maneuver_labels), ("topology", self.topology_labels)]:
            if len(arr) != total_frames:
                raise ValueError(
                    f"{name} count ({len(arr)}) != HDF5 frames ({total_frames}) for {short_id}"
                )
    ...
    def __getitem__(self, idx):
        ...
        for t in range(self.num_steps):
            ...
            if frame_idx < len(self.maneuver_labels):
                maneuver_ids.append(self.maneuver_labels[frame_idx])
            else:
                maneuver_ids.append(2)  # straight
            if frame_idx < len(self.topology_labels):
                topology_ids.append(self.topology_labels[frame_idx])
            else:
                topology_ids.append(0)

        sample["maneuver"] = torch.tensor(maneuver_ids, dtype=torch.long)
        sample["topology"] = torch.tensor(topology_ids, dtype=torch.long)
        return sample

    def __getattr__(self, name):
        if name in ("ds", "num_steps", "frameskip", "maneuver_labels",
                     "topology_labels", "offsets"):
            raise AttributeError(name)
        return getattr(self.ds, name)


def lejepa_dual_token_forward(self, batch, stage, cfg):
    """Forward with maneuver + topology token conditioning + depth loss."""
    ...
    # Add maneuver token embedding
    if "maneuver" in batch:
        maneuver_emb = self.model.maneuver_embedding(batch["maneuver"])
        act_emb = act_emb + maneuver_emb

    # Add topology token embedding
    if "topology" in batch:
        topology_emb = self.model.topology_embedding(batch["topology"])
        act_emb = act_emb + topology_emb
    ...
    if "depth_maps" in batch and depth_dec is not None:
        depth_pred = depth_dec(emb_flat).squeeze(1)
...
def run(cfg):
    ...
    # Wrap datasets with dual labels
    print("\nLoading maneuver + topology labels:")
    wrapped_train = []
    for ds in train_ds.datasets:
        w = DualTokenLabelWrapper(ds)
        wrapped_train.append(w)
        short_id = ds.h5_path.stem.split("_")[-1]
        print(f"  {short_id}: maneuver={len(w.maneuver_labels)}, topology={len(w.topology_labels)}, seqs={len(w)}")
    ...
    for ds in val_ds.datasets:
        ...
        short_id = ds.h5_path.stem.split("_")[-1]
        print(f"  {short_id}: maneuver={len(w.maneuver_labels)}, topology={len(w.topology_labels)} (val), seqs={len(w)}")
    ...
    # Topology token embedding (60 classes -> embed_dim)
    world_model.topology_embedding = nn.Embedding(NUM_TOPOLOGY_CLASSES, embed_dim)
    nn.init.normal_(world_model.topology_embedding.weight, std=0.02)
    print(f"Topology embedding: {NUM_TOPOLOGY_CLASSES} classes -> {embed_dim}D")
    ...
    world_model = spt.Module(
        ...
        forward=partial(lejepa_dual_token_forward, cfg=cfg),
        optim=optimizers,
    )
    ...
    run_id = cfg.get("subdir") or "track2_v2"
